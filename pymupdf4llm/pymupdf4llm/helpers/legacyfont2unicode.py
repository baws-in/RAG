...
def chanakya2unicode(text):
    """
    Converts text from the Chanakya font encoding to Unicode (Devanagari).

    This function is a direct port of a fragile JavaScript implementation.
    The order of replacements is critical and has been preserved.
    """

    # Mapping of Chanakya characters to Unicode characters.
    # The order of this list is crucial for the conversion logic.
    array_one = [
        # Repeated maatras and sequence change
        "aa", "a",
        "ZZ", "Z",
        "sas", "sa",
        "as", "sa",
    ...
    # --- Start of symbol replacement logic ---
    if not modified_substring:
        return ""

    # Initial specific replacements using regex
    modified_substring = re.sub('‘ª', 'ष्ट्र', modified_substring)
    modified_substring = re.sub('é', 'Â', modified_substring)
    modified_substring = re.sub('ÝQ', 'iQ', modified_substring)
    modified_substring = re.sub('Õk', ';', modified_substring)
    modified_substring = re.sub('sk', 'ks', modified_substring)

    # Move maatras before the space (this seems to be the intent)
    modified_substring = re.sub(
        r'([ ])([kzsSqwWa¡`±ZQ\+‚¨®sS©h¢ˇ%•∙·~ÈÊ›õ])',
        r'\2',
        modified_substring
    )

    # Adjust position of matras with special characters
    ...
    # Main replacement loop using the array_one map
    for i in range(0, len(array_one), 2):
        modified_substring = modified_substring.replace(array_one[i], array_one[i+1])

    # Adjust position of 'f' and 'Ż' (i-maatraas)
    modified_substring = re.sub('¯', 'Ż', modified_substring)
    modified_substring = re.sub('Ł', 'र्f', modified_substring)

    modified_substring = re.sub(
        r'([fŻ])([कखगघङचछजझञटठडढणतथदधनपफबभमयरलळवशषसहक़ख़ग़ज़ड़ढ़फ़य़ऱऩ])',
        r'\2\1',
        modified_substring
    )
    modified_substring = re.sub(
        r'([fŻ])((?:्[कखगघङचछजझञटठडढणतथदधनपफबभमयरलळवशषसहक़ख़ग़ज़ड़ढ़फ़य़ऱऩ])+)',
        r'\2\1',
        modified_substring
    )

    # Final conversion of i-maatraas
    modified_substring = re.sub('f', 'ि', modified_substring)
    modified_substring = re.sub('Ż', 'िं', modified_substring)

    # Adjust position of 'Z' (reph)
    modified_substring = re.sub('±', 'Zं', modified_substring)

    modified_substring = re.sub(
        r'([कखगघङचछजझञटठडढणतथदधनपफबभमयरलळवशषसहक़ख़ग़ज़ड़ढ़फ़य़ऱऩ])([ािीुूृेैोौंँ]*)([Z])',
        r'\3\1\2',
        modified_substring
    )
    modified_substring = re.sub(
        r'(([कखगघङचछजझञटठडढणतथदधनपफबभमयरलळवशषसहक़ख़ग़ज़ड़ढ़फ़य़ऱऩ]्)+)(Z)',
        r'\3\1',
        modified_substring
    )

    # Final conversion of reph
    modified_substring = re.sub('Z', 'र्', modified_substring)

    # Clean up wrongly typed maatras
    ...
    # Handle number-related conversions
...
def chanakya901_to_unicode(str_input):
    ...
    def replace_symbols(modified_substring):
        if modified_substring:
            modified_substring = re.sub(r'‘ª', r'ष्ट्र', modified_substring)
            ...
            modified_substring = re.sub(r'sk', r'ks', modified_substring)
...
result = chanakya2unicode(""" nqCkkjk """)
print(result)
#result = kruti2unicode("vki dSls lQy gksaxs\ ")
